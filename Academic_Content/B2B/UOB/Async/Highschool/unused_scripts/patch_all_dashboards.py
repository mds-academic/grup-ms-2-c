...
for repo in repos:
  ...
  return true;
    ...
    # First, let's see if goToStep exists. If not, add it before nextStep.
...
const nextStep = () => {
  ...
  if (currentStep.value < Object.keys(courseData).length) {
    ...
    mark_question_failed_regex = r'(const markQuestionFailed = \(qid\) => \{.*?)(revealQuizNext\(\);?\s*)(.*?\n\};)'
    content = re.sub(mark_question_failed_regex, r'\1\3', content, flags=re.DOTALL)
    
    # 5. Fix "attempts >= 3" blocks in handleStandardAnswer, submitMatchPairs, etc.
    # We will look for blocks like:
    # if (attempts >= 3) {
    #   ...
    # } else {
    def fix_attempts_block(match):
        inner_content = match.group(1)
        # remove revealQuizNext
        inner_content = re.sub(r'revealQuizNext\(.*?\);?', '', inner_content)
        # replace feedback text
        inner_content = re.sub(r'quizState\.value\.quizFeedback\s*=\s*".*?";?', 'quizState.value.quizFeedback = "Sudah 3 kali salah. Nilai checkpoint ini menjadi 0 dan modul berikutnya tetap terkunci.";', inner_content)
        # replace feedback text single quote
        inner_content = re.sub(r"quizState\.value\.quizFeedback\s*=\s*'.*?';?", 'quizState.value.quizFeedback = "Sudah 3 kali salah. Nilai checkpoint ini menjadi 0 dan modul berikutnya tetap terkunci.";', inner_content)
        # replace feedback text backtick
        inner_content = re.sub(r'quizState\.value\.quizFeedback\s*=\s*`.*?`;?', 'quizState.value.quizFeedback = "Sudah 3 kali salah. Nilai checkpoint ini menjadi 0 dan modul berikutnya tetap terkunci.";', inner_content)
        return f"if (attempts >= 3) {{\n{inner_content}}}"

    # Simple replacement: Any "quizFeedback =" inside "if (attempts >= 3)" block
    # It's safer to just explicitly replace it
    # But wait, python regex for matching braces is hard.
